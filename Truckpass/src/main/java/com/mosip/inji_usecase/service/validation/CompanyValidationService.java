...
public class CompanyValidationService implements ValidationService {
    ...
    private void loadConfig() {
        try {
            InputStream in = getClass().getClassLoader().getResourceAsStream("validation/company.json");
            if (in == null) {
                throw new RuntimeException("Config file not found: validation/company.json");
            }
            ...
            throw new RuntimeException("Failed to load config for CompanyValidationService", e);
        }
    }
    ...
    public void validate(Map<String, Object> data) {
        loadConfig();
        System.out.println("Called from CompanyValidationService");
        verifyFieldService.verifyRequired(data, requiredFields);
        verifyFieldService.verify(data, fields);
        System.out.println("CompanyValidationService: Validation Passed!");
    }
}
