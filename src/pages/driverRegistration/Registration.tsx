...
export const Registration: React.FC<RegistrationProps> = () => {
  ...
  const [selectedOpt, setSelectedOpt] = useState<EntryOption>('manualEntry');
  ...
  const handleLicenceNumChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setDriverLicenceNum(e.target.value);
  };

  const handlePassportNumChange = (e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          <div className="flex flex-col bg-[#FFFFFF] pt-5 pb-9 w-full px-6 rounded-br-2xl rounded-tr-2xl justify-between font-inter">
            ...
            <div className="space-y-4">
              ...
              <form className='flex flex-col gap-y-4' onSubmit={onSubmit}>
                {/* Disabled prefilled fields */}
                {[
                  { label: t('registration.fullName'), value: driverInfo?.name ?? '' },
                  { label: t('registration.uin'), value: '198765432123' },
                  { label: t('registration.gender'), value: driverInfo?.gender ?? '' },
                  { label: t('registration.eMailId'), value: driverInfo?.email ?? '' },
                  { label: t('registration.phNum'), value: driverInfo?.phone_number ?? '' },
                  { label: t('registration.city'), value: driverInfo?.address?.locality ?? '' },
                ].map((field, idx) => (
                  <div key={idx} className='space-y-1'>
                    ...
                    <input
                      ...
                      disabled
                      value={field.value}
                      className='bg-[#FAFAFA] text-[15px] text-[#717680] p-1.5 w-full border border-[#D5D7DA] rounded-md'
                    />
                  </div>
                ))}
                ...
                <div className='py-3 space-y-6'>
                  ...
                  {/* Licence Input + Options */}
                  <div className='space-y-3'>
                    <label className='flex items-center'>
                      <p className='text-xs text-[#414651]'>
                        {t('registration.driverLicenseNum')}
                    ...
                    <div className='flex gap-x-10'>
                      ...
                      <div className='flex items-center'>
                        <input
                          ...
                          onChange={handleEntryOptionChange}
                        />
                        <label htmlFor='shareViaInjiVerify' className='text-[#D5D7DA] px-1 text-sm'>
                          {t('registration.shareViaInjiVerify')}
                        </label>
                      </div>
                    </div>
                    ...
                    {selectedOpt === 'shareViaInjiVerify' && (
                      <div className={`flex flex-col ${licenseShared ? 'bg-[#EFFDF5] border-[#B3F6D2]' : 'bg-[#EEF7FF] border-[#B9DDFD]'} space-y-2 h-auto border rounded-lg p-4`}>
                        <h2 className={`text-sm font-semibold ${licenseShared ? 'text-[#007F41]' : 'text-[#006DE7]'}`}>
                          {licenseShared ? t('registration.fetchedSuccessfully') : t('registration.shareLicenseViaInjiVerify')}
                        </h2>
                        ...
                        {!licenseShared && (
                          <button
                            type="button"
                            onClick={shareViaInjiVerify}
                            className='bg-[#006DE7] w-[33%] text-[12px] font-[600] py-2.5 text-center rounded-[5px] text-[#FFFFFF] cursor-pointer'>
                    ...
                    <label className='flex items-center'>
                      <p className='text-sm text-[#414651]'>
                        {t('registration.cpc')}<span className='text-[#006DE7]'>*</span>
                      </p>
                      <img src={help_icon} alt='help_icon' className='h-3.5 cursor-pointer' />
                    </label>
                ...
                <div className='flex space-x-2 justify-end mt-5'>
                  ...
                  {selectedOpt !== 'shareViaInjiVerify' ? (
                    <button
                      type='submit'
                      disabled={!passportNum || !driverLicenceNum || !certificateUploaded}
                    ...
                    <button
                      type='submit'
                      disabled={!passportNum || !driverLicenceNum || !licenseShared || !certificateUploaded}
