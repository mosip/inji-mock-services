  ...
  useTranslation: () => ({
    t: (key: string) => {
      const translations: Record<string, string> = {
        'selectCompany.companyNames.transGlobal1': 'TransGlobal Company 1',
        'selectCompany.companyNames.transGlobalLogistics': 'TransGlobal Logistics',
        'selectCompany.companyNames.transGlobal2': 'TransGlobal Company 2',
        'selectCompany.companyNames.transGlobal3': 'TransGlobal Company 3',
        'selectCompany.companyNames.transGlobal4': 'TransGlobal Company 4',
        'selectCompany.companyNames.globalExpress': 'Global Express Transport',
        'selectCompany.companyNames.internationalFreight': 'International Freight',
        'selectCompany.activeAndValid': 'Active and Valid',
        'selectCompany.commercialTransport': 'Commercial Transport',
        'commans.continue': 'Continue',
        'selectCompany.selectRegisteredTransportCompany': 'Select Registered Transport Company',
        'selectCompany.chooseCompanyDesc': 'Choose your transport company from the list',
        'selectCompany.description': 'Search Transport Company',
        'selectCompany.inputPlaceholder': 'Search Transport Company',
        'selectCompany.companyDetails': 'Company Details',
      };
...
describe('SelectCompany Component', () => {
  ...
  test('renders select company section correctly', () => {
    ...
    expect(screen.getByText('Select Registered Transport Company')).toBeInTheDocument();
    expect(screen.getByText('Choose your transport company from the list')).toBeInTheDocument();
  ...
  test('filters companies based on search input', async () => {
    ...
    await waitFor(() => {
      // It should show companies with 'TransGlobal' in the name
      expect(screen.getAllByText(/TransGlobal/).length).toBeGreaterThan(0);
      // Global Express Transport shouldn't show for 'TransGlobal' search
