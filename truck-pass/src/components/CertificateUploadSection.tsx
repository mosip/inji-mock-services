...
export const CertificateUploadingSection: React.FC<CertificateUploadingSectionProps> = ({
  ...
  useEffect(() => {
    ...
    } else if (!propShowUploadingBlock) {
      setLocalShowUploadingBlock(false);
      setShowUploadingBlock(false);
    }
  }, [fileName, localErrorMsg, setShowUploadingBlock, propShowUploadingBlock]);
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      const uploadSimulation = setInterval(() => {
        setProgress((prev) => {
          const newProgress = Math.min(prev + 20, 100); // Cap at 100
          if (newProgress >= 100) {
    ...
    <div className="flex flex-col">
      ...
      >
        ...
        {localShowUploadingBlock && !localErrorMsg ? (
          <div
            role="region"
            aria-label="upload block"
            className="flex items-center justify-between w-full h-[4.5rem] border border-[#E4E7EC] rounded-md px-2.5 mt-12"
          >
            <div className="flex items-center space-x-2 w-full">
              ...
              <div className="flex flex-col mt-5 w-full">
                <p className="text-[#344054] text-[0.7rem] font-semibold">{fileName || 'CPC.pdf'}</p>
                <p className="text-[0.6rem] text-[#475467] font-semibold">
                  {fileSize ? Number(fileSize).toFixed(2) : '0'} KB
                </p>
          ...
          localErrorMsg && (
            <div
              role="region"
              aria-label="error block"
              className="flex items-center justify-between w-full h-[4.5rem] border border-[#FDA29B] rounded-md px-2.5 mt-12"
            >
              <div className="flex items-center space-x-2 w-full">
                <div className="items-center -mt-3">
                  <img src={file_type_icon} className="h-7 w-7" alt="file type icon" />
